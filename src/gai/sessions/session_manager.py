...
class LocalSessionManager(Generic[T]):
    ...
    async def __aexit__(self, exc_type, exc, tb):
        if exc_type:
            logger.warning(
                f"GenericSessionManager: __aexit__ triggered due to exception. exc_type={exc_type}, exc={exc}"
            )
        else:
            logger.debug("GenericSessionManager: __aexit__ triggered normally.")

        try:
            await self.stop()
        except Exception as e:
            logger.error(f"GenericSessionManager: Error in __aexit__: {e}")
    ...
    async def start(self):
        ...
        logger.info("GenericSessionManager: ready.")

    async def stop(self):
        """Stop the dialogue manager."""
        logger.debug("GenericSessionManager: Stopping...")

        try:
            if self.bus:
                await self.bus.stop()
        except Exception as e:
            logger.error(f"GenericSessionManager: Error during stop: {e}")
        finally:
            logger.info("GenericSessionManager: stopped.")
    ...
    async def subscribe(self, subject: str, callback: dict[str, Callable]):
        ...
        if not isinstance(callback, dict):
            raise TypeError(
                "GenericSessionManager: subscribe() requires a dictionary of callbacks."
            )
    ...
    async def publish(
        ...
        *,
        ...
        try:
            ...
            logger.error(f"GenericSessionManager: Error during publish: {e}")
            raise
    ...
    def _prepare_message(
        ...
        if pydantic:
            ...
            raise TypeError(
                "GenericSessionManager: publish() requires either `pydantic` or all of `type`, `body`, `sender`, and `recipient` as keyword arguments."
            )
    ...
    def log_message(self, pydantic: T):
        ...
        logger.debug(f"GenericSessionManager: message={pydantic}")
        ...
        if any(m for m in self.messages if pydantic.id == m.id):
            logger.debug(
                f"GenericSessionManager: message with id={pydantic.id} already logged, skipping."
            )
            return
        ...
        try:
            ...
            logger.error(f"Failed to persist message to storage: {e}")
    ...
    def reset(self):
        ...
        self.messages.clear()
        logger.debug("GenericSessionManager: Dialogue reset. all messages cleared.")
    ...
    def delete_message(self, message_id: str):
        ...
        logger.debug(
            f"GenericSessionManager: Message deleted. message_id= {message_id}"
        )
