...
class Dialogue:
    def __init__(
        ...
        self._messages:MessagePydantic = []
    ...
    def add_assistant_message(self, sender:str, chunk:str, content:Optional[str]=None) -> MessagePydantic:
    ...
    def extract_recap(self, last_n: int=0) -> str:
        return message_helper.extract_recap(
            self._messages,
            last_n=last_n,
            max_recap_size=self.max_recap_size
            )
...
class FileDialogue(Dialogue):
    ...
    def __init__(self, 
        ...
        file_path = file_path or f"~/.gai/data/{self.caller_id}/User/dialogues/{self.dialogue_id}.json"
        self.file_path = os.path.expanduser(file_path)
        self.message_store = MessageStore[MessagePydantic](file_path=self.file_path, MessagePydantic_cls=MessagePydantic)
