...
class MessageStore(Generic[MessagePydanticT]):
    ...
    def get_message(self, message_id: str) -> Optional[MessagePydanticT]:
        ...
        with MessageStore.file_lock:
            if not os.path.exists(self.file_path):
                logger.error(f"DialogueFileStorage: Dialogue file not found. path={self.file_path}")
                raise FileNotFoundError(f"DialogueFileStorage: Dialogue file not found. path={self.file_path}")

            with open(self.file_path, "r") as f:
                try:
                    internal_structure = MessageStore.InternalStructure(**json.load(f))
                except json.JSONDecodeError:
                    logger.warning(f"DialogueFileStorage: Failed to load internal structure from {self.file_path}. Creating new one.")
    ...
    def list_messages(self) -> list[MessagePydanticT]:
        ...
        with MessageStore.file_lock:
            if not os.path.exists(self.file_path):
                logger.warning(f"DialogueFileStorage: Dialogue file not found. path={self.file_path}")
                raise FileNotFoundError(f"DialogueFileStorage: Dialogue file not found path={self.file_path}")
            ...
            with open(self.file_path, "r") as f:
                try:
                    ...
                    logger.error(f"DialogueFileStorage: Failed to load internal structure from {self.file_path}. Creating new one.")
                    raise Exception(f"DialogueFileStorage: Failed to load internal structure from {self.file_path}. Creating new one.")
    ...
    def bulk_insert_messages(self, messages: list):
        """Insert multiple messages into the dialogue file."""
        with MessageStore.file_lock:
            if not messages:
                logger.warning("DialogueFileStorage: No messages to insert.")
                return
            ...
            logger.debug(f"DialogueFileStorage: {len(messages)} messages added to file.")
    ...
    def delete_message(self, id: str):
        ...
        with MessageStore.file_lock:
            ...
            internal_structure.messages = [msg for msg in internal_structure.messages if msg["id"] != id]
            if len(internal_structure.messages) == previous_length:
                logger.warning(f"DialogueFileStorage: Message with id={id} not found in file. No deletion performed.")
                return
        ...
        with MessageStore.file_lock:
            ...
            for i, msg in enumerate(internal_structure.messages):
                if msg["id"] == message.id:
                    internal_structure.messages[i] = message.model_dump()  # ✅ REPLACE instead of mutating
                    updated = True
                    break
