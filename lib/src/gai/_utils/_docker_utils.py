...
def _docker_push(pyproject_path, repo_name, image_name):
    # Validate pyproject_path
    if not pyproject_path:
        console.print("[red]pyproject_path cannot be empty.[/]")
        return
    pyproject_path = os.path.abspath(pyproject_path)
    if not pyproject_path.endswith("pyproject.toml"):
        console.print(f"[red]{pyproject_path}[/] does not end with 'pyproject.toml'")
    ...
    # Extract image_name from pyproject if not provided
    if not image_name:
    ...
    dockerfile_dir = os.path.dirname(pyproject_path)

    console.print(f"[blue]Building and pushing multi-arch image:[/] [green]{versioned_image}[/], [green]{latest_image}[/]")

    _cmd(f"""docker buildx build \
        --tag {versioned_image} \
        --tag {latest_image} \
        --push \
        -f {os.path.join(dockerfile_dir, 'Dockerfile')} \
        {dockerfile_dir}""")
    # _cmd(f"""docker buildx build \
    #     --platform=linux/amd64,linux/arm64 \
    #     --tag {versioned_image} \
    ...
    console.print(f"[green bold]Push {versioned_image} and {latest_image} Done[/]")
